...
class CorrelationTracker(Tracker):
    ...
    def __init__(self, detection_rate=5, **kwargs):
        ...
        # Previous frame (used in cross correlation algorithm)
    ...
    def build_tracklet(self, frame):
        ...
        prev_gray = rgb2gray(self.prev_frame)
        ...
        for label, xmin, ymin, xmax, ymax in self.detection_history[-1]:
            ...
            detections_frame = extract_detections(frame_gray)
            detections_prev = extract_detections(prev_gray)
            ...
            current_box = np.argmax(matching * gaussian(prev_gray.shape,
                                                        frame_gray.mean(),
                                                        prev_gray.mean(),
                                                        frame_gray.std(),
                                                        prev_gray.std()))
